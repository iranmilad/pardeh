<!--            Search Form:start-->
<div class="main-search position-relative tw-w-2/5 border-0 border-radius-xl bg-gray-150">
    <form class="tw-flex">
        <button class="btn" type="submit">
            <i class="fab fa-sistrix fw-md fs-5 gray-500"></i>
        </button>
        <input wire:model.live="searchTerm" class="form-control border-0 tw-bg-transparent fs-7 focus:tw-bg-transparent" type="text" placeholder="جستجو" id="mainSearchInput" autocomplete="off">
        @if(!empty($results))
        <div class="searchButton " wire:loading.class="searchLoading" wire:target="searchTerm">
            <button class="btn" wire:click="clear" type="button" id="mainSearchClose">
                <i class="fas fa-times fw-md fs-5 gray-500"></i>
            </button>
            <button type="button" class="btn loadingBtn">
                <svg class="tw-animate-spin -tw-ml-1 tw-mr-3 tw-h-5 tw-w-5 tw-text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="#9ca3af" stroke-width="4"></circle>
    ...
    <div class="search-results custom-box-shadow-s-2 {{ !empty($results->products) ? 'd-block' : '' }}">
        ...
        <a href="#" class="search-result-item position-relative border-bottom p-3 text-dark ">
            <img src="{{ $product->thumbnail }}" class="tw-w-16 tw-h-16 fw-md fs-5 gray-500 tw-block tw-relative tw-rounded-md" alt="{{ $product->name }}">
